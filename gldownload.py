...
def trySremsrc(imdbId):
    ...
    if len(streams_json):
        ...
        command = (
            ...
            f'--concurrent-fragments 8 '
            f'--abort-on-unavailable-fragment '
            f'-o blob "https://solitary-grass-77bc.hostproxy.workers.dev/{playlist_url}"'
        )
        output = os.system(
            command,
        )
...
def tryTorrentio(imdbId):
    stream_url = ""
    if ":" in imdbId:
        stream_url = "https://torrentio.strem.fun/qualityfilter=4k%7Csizefilter=3GB%7Cdebridoptions=nodownloadlinks,nocatalog%7Ctorbox={}/stream/series/{}.json".format(
            TB_API_KEY,
            imdbId
        )
    else:
